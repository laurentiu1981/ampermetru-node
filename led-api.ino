...
int ledControl(String command)
{
    ...
    // command = "{CPU|MEM|HDD}|<cpu_value>|<mem_value>|<hdd_value>|<ssh_value>|<hostname>"
    ...
    if (isCpu) {
        ...
        ampermeterValue = resetAmpermeterValue(parameterCpu);
        errorCount = 0;
    }
    if (isMemory) {
        memLedValue = HIGH;
        activeMemCpuSsh = memLed;
        ampermeterValue = resetAmpermeterValue(parameterMem);
        errorCount = 0;
    }
    if (isHdd) {
        hddLedValue = HIGH;
        activeMemCpuSsh = hddLed;
        ampermeterValue = resetAmpermeterValue(parameterHdd);
        errorCount = 0;
    }
...
// percentage = 0..100
int resetAmpermeterValue(float percentage)
{
